...
void	set_texture(t_cub *cub, t_camera *cam)
{
	...
	i = cub->anim_frame;
	if (cam->is_door && cub->doors_open == 0)
	{
		cam->tex = cub->textures.dor;
		return ;
	}
	if (cam->side == 0)
	{
		if (cam->ray_dir_x > 0)
			cam->tex = cub->textures.we[i % 3];
		else
			cam->tex = cub->textures.ea[i % 3];
	}
	else
	{
		if (cam->ray_dir_y > 0)
			cam->tex = cub->textures.no[i % 3];
		else
			cam->tex = cub->textures.so[i % 3];
	}
}
